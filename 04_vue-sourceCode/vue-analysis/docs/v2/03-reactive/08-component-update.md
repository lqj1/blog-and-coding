...
在组件化章节，我们介绍了 Vue 的组件化实现过程，不过我们只讲了 Vue 组件的创建过程，并没有涉及到组件数据发生变化，更新组件的过程。而通过我们这一章对数据响应式原理的分析，了解到当**数据发生变化的时候，会触发渲染 `watcher` 的回调函数，进而执行组件的更新过程**，接下来我们来详细分析这一过程。
...
组件更新的过程核心就是**新旧 vnode diff**，对新旧节点相同以及不同的情况分别做不同的处理。
